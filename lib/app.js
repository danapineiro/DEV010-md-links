...
fileReader.on("line", function (line) {  // Este controlador de eventos lee el archivo línea por línea.
    ...
    if (validUrls.length) {  // Si el array no está vacío, lo enviamos a la función que realizará la solicitud GET (validateLinks).
        isLocalFile(validUrls).then((res) => console.log(res));
    }
});
...
const isLocalFile = (urls) => new Promise((resolve, reject) => { //a la funcion is localfile le paso un array de URLS
    const promiseArray = urls.map((url) => axios.get(url) //con anxios.get hago la llamada GET a la URL . Esto guarda un array de promesas
