...
/*const getURLStatus = (url) => {
    return axios.get(url, { timeout: 5000 }) // Establece un tiempo de espera de 5 segundos
      .then((response) => {
        return {
          url,
          status: response.status,
      ...
      .catch((error) => {
        if (error.response) {
          // Si hay una respuesta de error en el objeto error
          return {
            url,
          ...
          // Si no hay una respuesta de error en el objeto error, maneja el error de otra manera
          return {
            url,
            status: "Error: " + error.message,
            ok: "fail",
